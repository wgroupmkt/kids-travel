...
export async function POST(req: Request) {
  try {
    ...
    if (!verifyData.success) {
      console.error("❌ Error Turnstile:", verifyData);
    ...
    // 🔒 VALIDACIÓN: máximo 15 participaciones por mes
    const now = new Date();
    ...
    let raffleNumber = Date.now().toString().slice(-6);
    ...
    // 🔢 ACTUALIZAR CONTADOR
    const currentTotal =
      passengerDoc.data()?.totalParticipants || 0;
    ...
    // 🖼 GENERAR IMAGEN
    const bufferImagen = await generarImagen(
      raffleNumber.toString()
    );
    ...
    if (email && resend) {
      try {
        await resend.emails.send({
          from: "Registro <info@bonokids.wolftravel.com.ar>",
          to: email,
          ...
          html: `
            <div style="font-family: Arial; padding:20px;">
              ...
              <p>
                Hola ${name}, gracias por ayudarme con mi viaje
                de egresados 💙🐺
              </p>
      ...
      console.log(
        "⚠️ Email no enviado (falta email o API key)"
      );
    }
